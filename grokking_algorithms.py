# -*- coding: utf-8 -*-
"""Grokking Algorithms.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1IajGk6E_AuEdobL7rXUWmkJ58lfO-ZFW

# Алгоритмы из книги "Грокаем алгоритмы"

## Бинарный поиск -
это алгоритм, который на вход получает отсортированный список элементов. Если элемент, который Вы ищите, присутствует в списке, он возвращает ту позицию, в которой он был найден. В противном случае возвращает null.
...
**Связанные списки** отлично подходят в тех случаях, когда данные должны читаться **последовательно**: сначала вы читаете один элемент, потом по адресу переходите к следующему элементу. В связанном списке элементы не хранятся рядом друг с другом, поэтому мгновенно определить i-го элемента в памяти невозможно - нужно обратиться к первому элементу, чтобы получить адрес второго элемента, затем обратиться ко второму элементу для того, чтобы получить адрес третьего - и так далее.
